...
namespace paddle {
    namespace lite_api {
        ...
        enum class LiteModelType {
            kProtobuf = 0, kNaiveBuffer, UNK
        };
        ...
        public:
            ...
            virtual std::shared_ptr<PaddlePredictor> Clone(const std::vector<std::string> &var_names) = 0;
            ...
            virtual void SaveOptimizedModel(const std::string &model_dir, LiteModelType model_type = LiteModelType::kProtobuf, bool record_info = false);
        ...
        class LITE_API ConfigBase {
            ...
            std::map<std::string, std::pair<std::vector<char>, std::vector<char>>> subgraph_model_cache_buffers_{};
            ...
            int (*nnadapter_context_callback_)(int event_id, void *user_data){nullptr};  // NOLINT
            ...
            std::map<std::string, std::vector<std::vector<int64_t>>> nnadapter_dynamic_shape_info_;
        ...
        public:
            ...
            /// \return void
            void set_opencl_binary_path_name(const std::string &path, const std::string &name);
            ...
            /// \return void
            void set_opencl_tune(CLTuneMode tune_mode = CL_TUNE_NONE, const std::string &path = "", const std::string &name = "", size_t lws_repeats = 4);
            ...
            void set_subgraph_model_cache_buffers(const std::string &key, const std::vector<char> &cfg, const std::vector<char> &bin);

            const std::map<std::string, std::pair<std::vector<char>, std::vector<char>>> &subgraph_model_cache_buffers() const {
            ...
            //   return 0;
            // }
            void set_nnadapter_context_callback(int (*nnadapter_context_callback)(int event_id, void *user_data)) {
            ...
            const std::map<std::string, std::vector<std::vector<int64_t>>> &nnadapter_dynamic_shape_info() const {
            ...
            const std::map<std::string, std::vector<char>> &nnadapter_model_cache_buffers() const {
            ...
            void set_xpu_conv_autotune(bool autotune = true, const std::string &autotune_file = "");
            ...
            void set_xpu_multi_encoder_method(const std::string &precision = "int16", bool adaptive_seqlen = false);
            ...
            std::string metal_lib_path() const {
                return metal_path_;
            }

            bool metal_use_mps() const {
                return metal_use_mps_;
            }

            bool metal_use_aggressive() const {
                return metal_use_aggressive_;
            }
            ...
            bool metal_use_memory_reuse() const {
                return metal_use_memory_reuse_;
            }
            ...
            void set_custom_allocator(TargetType target_type, CustomAllocator custom_allocator);
        };

        class LITE_API CxxModelBuffer {
        public:
            CxxModelBuffer(const char *program_buffer, size_t program_buffer_size, const char *params_buffer, size_t params_buffer_size);
        ...
        class LITE_API CxxConfig : public ConfigBase {
            ...
            std::map<int, std::vector<std::shared_ptr<void>>> preferred_inputs_for_warmup_;
        ...
        public:
            ...
            void set_model_buffer(const char *model_buffer, size_t model_buffer_size, const char *param_buffer, size_t param_buffer_size) {
                model_buffer_.reset(new CxxModelBuffer(model_buffer, model_buffer_size, param_buffer, param_buffer_size));
            }
            ...
            // for comman users.
            void set_passes_internal(const std::vector<std::string> &passes_internal = {}) {
            ...
            bool is_model_from_memory() const {
                return static_cast<bool>(model_buffer_);
            }
            ...
            template<class T>
            void set_preferred_inputs_for_warmup(const int group_idx, const int tensor_idx, const shape_t &shape, const lod_t &lod = {}, const T fill_value = 0, const void *data = nullptr);

            const std::map<int, std::vector<std::shared_ptr<void>>> &preferred_inputs_for_warmup() const {
                return preferred_inputs_for_warmup_;
            }

            void set_quant_model(bool quant_model) {
                quant_model_ = quant_model;
            }
            ...
            void set_sparse_model(bool sparse_model) {
                sparse_model_ = sparse_model;
            }

            bool sparse_model() const {
                return sparse_model_;
            }
            ...
            void set_nnadapter_mixed_precision_quantization_config_path(const std::string &mixed_precision_quantization_config_path) {
                mixed_precision_quantization_config_path_ = mixed_precision_quantization_config_path;
            }

            const std::string &nnadapter_mixed_precision_quantization_config_path() const {
            ...
            void set_nnadapter_mixed_precision_quantization_config_buffer(const std::string &mixed_precision_quantization_config_buffer) {
                mixed_precision_quantization_config_buffer_ = mixed_precision_quantization_config_buffer;
            }

            const std::string &nnadapter_mixed_precision_quantization_config_buffer() const {
        ...
        public:
            ...
            const std::string &lite_model_file() const {
                return lite_model_file_;
            }

            // return model buffer data, which is in combined format.
            const char *lite_model_buffer_ptr() const {
                return lite_model_buffer_ptr_;
            }

            size_t lite_model_buffer_size() const {
                return lite_model_buffer_size_;
            }

            // return model_from_memory_, which indicates whether to load model from
            // memory buffer.
            bool is_model_from_memory() const {
                return model_from_memory_;
            }
            ...
            void SetArmL3CacheSize(L3CacheSetMethod method = L3CacheSetMethod::kDeviceL3Cache, int absolute_val = -1);
