...
namespace paddle {
    namespace lite_api {
        ...
        enum class TargetType : int {
            kUnk = 0, kHost = 1, kX86 = 2, kCUDA = 3, kARM = 4, kOpenCL = 5, kAny = 6,  // any target
            kFPGA = 7, kNPU = 8, kXPU = 9, kBM = 10, kMLU = 11, kRKNPU = 12, kAPU = 13, kHuaweiAscendNPU = 14, kImaginationNNA = 15, kIntelFPGA = 16, kMetal = 17, kNNAdapter = 18, NUM = 19,  // number of fields.
        };
        enum class PrecisionType : int {
            kUnk = 0, kFloat = 1, kInt8 = 2, kInt32 = 3, kAny = 4,  // any precision
            kFP16 = 5, kBool = 6, kInt64 = 7, kInt16 = 8, kUInt8 = 9, kFP64 = 10, NUM = 11,  // number of fields.
        };

        typedef enum {
            LITE_PRECISION_NORMAL = 0, LITE_PRECISION_LOW = 1,
        } PrecisionMode;

        enum class DataLayoutType : int {
            kUnk = 0, kNCHW = 1, kNHWC = 3, kImageDefault = 4,  // for opencl image2d
            kImageFolder = 5,   // for opencl image2d
            kImageNW = 6,       // for opencl image2d
            kAny = 2,           // any data layout
            kMetalTexture2DArray = 7, kMetalTexture2D = 8, NUM = 9,  // number of fields.
        };

        typedef enum {
            LITE_POWER_HIGH = 0, LITE_POWER_LOW = 1, LITE_POWER_FULL = 2, LITE_POWER_NO_BIND = 3, LITE_POWER_RAND_HIGH = 4, LITE_POWER_RAND_LOW = 5
        } PowerMode;

        typedef enum {
            CL_TUNE_NONE = 0, CL_TUNE_RAPID = 1, CL_TUNE_NORMAL = 2, CL_TUNE_EXHAUSTIVE = 3
        } CLTuneMode;

        typedef enum {
            CL_PRECISION_AUTO = 0, CL_PRECISION_FP32 = 1, CL_PRECISION_FP16 = 2
        } CLPrecisionType;
        ...
        struct PrecisionTypeTrait {
            constexpr static PrecisionType Type() {
...
#define DATALAYOUT(item__) paddle::lite_api::DataLayoutType::item__
        ...
        std::set<PrecisionType> ExpandValidPrecisions(PrecisionType precision = PRECISION(kAny));

        // Get a set of all the elements represented by the layout.
        std::set<DataLayoutType> ExpandValidLayouts(DataLayoutType layout = DATALAYOUT(kAny));
        ...
        struct LITE_API Place {
            ...
            Place(TargetType target, PrecisionType precision = PRECISION(kFloat), DataLayoutType layout = DATALAYOUT(kNCHW), int16_t device = 0) : target(target), precision(precision), layout(layout), device(device) {
            }

            bool is_valid() const {
                return target != TARGET(kUnk) && precision != PRECISION(kUnk) && layout != DATALAYOUT(kUnk);
            }

            size_t hash() const;

            bool operator==(const Place &other) const {
                return target == other.target && precision == other.precision && layout == other.layout && device == other.device;
            }
