...
#define NEAR_ZERO(val) (((val) > -TOLERANCE) && ((val) < TOLERANCE))
    ...
    bool SlopesEqual(const TEdge &e1, const TEdge &e2, bool UseFullInt64Range) {
        ...
        if (UseFullInt64Range)
            return Int128Mul(e1.Top.Y - e1.Bot.Y, e2.Top.X - e2.Bot.X) == Int128Mul(e1.Top.X - e1.Bot.X, e2.Top.Y - e2.Bot.Y);
        else
#endif
    ...
    throw clipperException("AddPath: Open paths have been disabled.");
        ...
        for (;;) {
            ...
            else if (Closed && SlopesEqual(E->Prev->Curr, E->Curr, E->Next->Curr, m_UseFullRange) && (!m_PreserveCollinear || !Pt2IsBetweenPt1AndPt3(E->Prev->Curr, E->Curr, E->Next->Curr))) {
...
//------------------------------------------------------------------------------
    ...
    OutPt *Clipper::AddLocalMinPoly(TEdge *e1, TEdge *e2, const IntPoint &Pt) {
        ...
        if (prevE && prevE->OutIdx >= 0 && prevE->Top.Y < Pt.Y && e->Top.Y < Pt.Y) {
            ...
            cInt xE = TopX(*e, Pt.Y);
            if (xPrev == xE && (e->WindDelta != 0) && (prevE->WindDelta != 0) && SlopesEqual(IntPoint(xPrev, Pt.Y), prevE->Top, IntPoint(xE, Pt.Y), e->Top, m_UseFullRange)) {
    ...
    void Clipper::InsertLocalMinimaIntoAEL(const cInt botY) {
        ...
        while (PopLocalMinima(botY, lm)) {
            ...
            if (lb->OutIdx >= 0 && lb->PrevInAEL && lb->PrevInAEL->Curr.X == lb->Bot.X && lb->PrevInAEL->OutIdx >= 0 && SlopesEqual(lb->PrevInAEL->Bot, lb->PrevInAEL->Top, lb->Curr, lb->Top, m_UseFullRange) && (lb->WindDelta != 0) && (lb->PrevInAEL->WindDelta != 0)) {
            ...
            if (lb->NextInAEL != rb) {
                ...
                if (rb->OutIdx >= 0 && rb->PrevInAEL->OutIdx >= 0 && SlopesEqual(rb->PrevInAEL->Curr, rb->PrevInAEL->Top, rb->Curr, rb->Top, m_UseFullRange) && (rb->WindDelta != 0) && (rb->PrevInAEL->WindDelta != 0)) {
...
//------------------------------------------------------------------------------
        ...
        if (e1->WindDelta == 0 || e2->WindDelta == 0) {
            ...
            if (e1->WindDelta == 0 && e2->WindDelta == 0)
                ...
                if ((e1->WindDelta == 0) && abs(e2->WindCnt) == 1 && (m_ClipType != ctUnion || e2->WindCnt2 == 0)) {
                    ...
                    if (e1Contributing)
                        e1->OutIdx = Unassigned;
                } else if ((e2->WindDelta == 0) && (abs(e1->WindCnt) == 1) && (m_ClipType != ctUnion || e1->WindCnt2 == 0)) {
                    AddOutPt(e2, Pt);
                    if (e2Contributing)
                        e2->OutIdx = Unassigned;
                }
    ...
    void Clipper::ProcessHorizontal(TEdge *horzEdge) {
        ...
        if (horzEdge->NextInLML) {
            if (horzEdge->OutIdx >= 0) {
                ...
                if (ePrev && ePrev->Curr.X == horzEdge->Bot.X && ePrev->Curr.Y == horzEdge->Bot.Y && ePrev->WindDelta != 0 && (ePrev->OutIdx >= 0 && ePrev->Curr.Y > ePrev->Top.Y && SlopesEqual(*horzEdge, *ePrev, m_UseFullRange))) {
                    OutPt *op2 = AddOutPt(ePrev, horzEdge->Bot);
                    AddJoin(op1, op2, horzEdge->Top);
                } else if (eNext && eNext->Curr.X == horzEdge->Bot.X && eNext->Curr.Y == horzEdge->Bot.Y && eNext->WindDelta != 0 && eNext->OutIdx >= 0 && eNext->Curr.Y > eNext->Top.Y && SlopesEqual(*horzEdge, *eNext, m_UseFullRange)) {
    ...
    void Clipper::ProcessEdgesAtTopOfScanbeam(const cInt topY) {
        ...
        while (e) {
            if (IsIntermediate(e, topY)) {
                ...
                TEdge *ePrev = e->PrevInAEL;
                TEdge *eNext = e->NextInAEL;
                if (ePrev && ePrev->Curr.X == e->Bot.X && ePrev->Curr.Y == e->Bot.Y && op && ePrev->OutIdx >= 0 && ePrev->Curr.Y > ePrev->Top.Y && SlopesEqual(e->Curr, e->Top, ePrev->Curr, ePrev->Top, m_UseFullRange) && (e->WindDelta != 0) && (ePrev->WindDelta != 0)) {
                    OutPt *op2 = AddOutPt(ePrev, e->Bot);
                    AddJoin(op, op2, e->Top);
                } else if (eNext && eNext->Curr.X == e->Bot.X && eNext->Curr.Y == e->Bot.Y && op && eNext->OutIdx >= 0 && eNext->Curr.Y > eNext->Top.Y && SlopesEqual(e->Curr, e->Top, eNext->Curr, eNext->Top, m_UseFullRange) && (e->WindDelta != 0) && (eNext->WindDelta != 0)) {
    ...
    void Clipper::FixupOutPolygon(OutRec &outrec) {
        ...
        for (;;) {
            ...
            if ((pp->Pt == pp->Next->Pt) || (pp->Pt == pp->Prev->Pt) || (SlopesEqual(pp->Prev->Pt, pp->Pt, pp->Next->Pt, m_UseFullRange) && (!preserveCol || !Pt2IsBetweenPt1AndPt3(pp->Prev->Pt, pp->Pt, pp->Next->Pt)))) {
                lastOK = 0;
                OutPt *tmp = pp;
    ...
    void ClipperOffset::FixOrientations() {
        ...
        if (m_lowest.X >= 0 && !Orientation(m_polyNodes.Children[(int) m_lowest.X]->Contour)) {
            for (int i = 0; i < m_polyNodes.ChildCount(); ++i) {
    ...
    void OpenPathsFromPolyTree(PolyTree &polytree, Paths &paths) {
        ...
        for (int i = 0; i < polytree.ChildCount(); ++i)
            if (polytree.Children[i]->IsOpen())
                paths.push_back(polytree.Children[i]->Contour);
    }
    //------------------------------------------------------------------------------
