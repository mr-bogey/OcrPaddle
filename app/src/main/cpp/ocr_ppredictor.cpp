...
namespace ppredictor {
    ...
    int OCR_PPredictor::init(const std::string &det_model_content, const std::string &rec_model_content, const std::string &cls_model_content) {
        _det_predictor = std::make_unique<PPredictor>(_config.use_opencl, _config.thread_num, NET_OCR, _config.mode);
        _det_predictor->init_nb(det_model_content);

        _rec_predictor = std::make_unique<PPredictor>(_config.use_opencl, _config.thread_num, NET_OCR_INTERNAL, _config.mode);
        _rec_predictor->init_nb(rec_model_content);

        _cls_predictor = std::make_unique<PPredictor>(_config.use_opencl, _config.thread_num, NET_OCR_INTERNAL, _config.mode);
        _cls_predictor->init_nb(cls_model_content);
        return RETURN_OK;
    }

    int OCR_PPredictor::init_from_file(const std::string &det_model_path, const std::string &rec_model_path, const std::string &cls_model_path) {
        _det_predictor = std::make_unique<PPredictor>(_config.use_opencl, _config.thread_num, NET_OCR, _config.mode);
     ...
     * @param srcimg
     */
//    static void visual_img(const std::vector<std::vector<std::vector<int>>> &filter_boxes, const std::vector<std::vector<std::vector<int>>> &boxes, const cv::Mat &srcimg) {
//        // visualization
...
//    }
    ...
    std::vector<OCRPredictResult> OCR_PPredictor::infer_ocr(cv::Mat &origin, int max_size_len, int run_det, int run_cls, int run_rec) {
        LOGI("ocr cpp start *****************");
    ...
    void OCR_PPredictor::infer_det(cv::Mat &origin, int max_size_len, std::vector<OCRPredictResult> &ocr_results) {
        ...
        const auto *dimg = reinterpret_cast<const float *>(input_image.data);
        ...
        std::vector<std::vector<std::vector<int>>> filtered_box = calc_filtered_boxes(res.get_float_data(), res.get_size(), input_image.rows, input_image.cols, origin);
    ...
    void OCR_PPredictor::infer_rec(const cv::Mat &origin_img, int run_cls, OCRPredictResult &ocr_result) {
        ...
        LOGI("predict_batch size %ld", results.at(0).get_size());
        LOGI("predict_batch %f %f", predict_batch[0], predict_batch[1]);
        const std::vector<int64_t> predict_shape = results.at(0).get_shape();
        LOGI("predict_shape %ld %ld", predict_shape[0], predict_shape[1]);

        if (predict_shape.size() < 3) {
            ocr_result.score = 0.f;
            return;
        }
        ...
        for (int n = 0; n < predict_shape[1]; n++) {
            auto first_value = &predict_batch[n * predict_shape[2]];
            auto last_value = &predict_batch[(n + 1) * predict_shape[2]];

            argmax_idx = int(argmax(first_value, last_value));
            max_value = float(*std::max_element(first_value, last_value));
        ...
        LOGI("ocr cpp rec word size %d", count);
    }
    ...
    std::vector<std::vector<std::vector<int>>> OCR_PPredictor::calc_filtered_boxes(const float *pred, int pred_size, int output_height, int output_width, const cv::Mat &origin) {
        ...
        std::vector<std::vector<std::vector<int>>> filter_boxes = filter_tag_det_res(boxes, ratio_h, ratio_w, origin);
        return filter_boxes;
    }
    ...
    float OCR_PPredictor::postprocess_rec_score(const PredictorOutput &res) {
        ...
        for (int n = predict_lod[0][0]; n < predict_lod[0][1] - 1; n++) {
            int argmax_idx = argmax(predict_batch + n * predict_shape[1], predict_batch + (n + 1) * predict_shape[1]);
